...
public class CollationDatabaseTests : BaseDatabaseTests
{
    ...
    {
        var results = SystemConsole.Invoke("pwsh", $"-Command dotnet SqlTest.dll runall --image {image} --project {this.Folder}/Collation --collation Polish_CI_AS; echo $LASTEXITCODE");
    ...
    {
        var results = SystemConsole.Invoke("pwsh", $"-Command dotnet SqlTest.dll runall --image {image} --project {this.Folder}/Collation; echo $LASTEXITCODE");
